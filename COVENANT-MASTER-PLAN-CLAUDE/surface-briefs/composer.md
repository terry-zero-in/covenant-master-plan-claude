...
Synthesis: Composer is original because no benchmark composes a **lender-shaped regulatory deliverable from deterministic engine values under a computed gate**: Releases contributes the draft→freeze spine, branch protection the named-reasons gate, Vanta/Ironclad the evidence-freshness and verification grammar, DataSnipper the self-contained provenance, Numeric the draft-approve narrative rhythm — and the domain shape (the SLOT-4 exemplar: statements + rent schedule + questionnaire + certification + attachments) comes from the real submitted package, not from any of them. The result is domain-correct for borrower-side lender reporting precisely because the surface's unit of work is the lender's own form field, its numbers are the engine's, and its gate is the credit agreement's checklist made computable — a combination none of the referenced products needs and none offers.

Ceremony-budget note (NN/g, R4): Composer deliberately carries **zero** heavy ceremony — approvals, answers, and pins are one-gesture inline acts — so that the two typed ceremonies downstream (certify, send) stay rare, distinct, and meaningful. Any future proposal to add a confirmation dialog to this surface fails review by default.
