...
- `InboxPreview` — NEW summoned inspector. Parts: identity header (class icon, title, loan+period chips, deadline chip); per-class context body (§8 region 3); event trail (this item's ActivityEvents); footer action bar. Read-only except the three acts.
